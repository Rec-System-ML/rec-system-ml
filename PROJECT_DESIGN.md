...
```
back/
├── api.py                   # FastAPI 服务入口，注册路由
├── train.py                 # 模型训练脚本，生成 artifact
├── models/
...
│   ├── cold_start.py        # /api/cold-start/recommend
│   └── stats.py             # /api/stats
└── utils/
    ├── loader.py            # artifact 加载、全局状态、对外工具函数
    ├── pipeline.py          # 数据读取与清洗
    ├── evaluation.py        # HR@K / NDCG@K 评估指标
    └── tags.py              # tag_id → 名称映射
```
...
│   ├── interest_evolution_data.js  # 预生成数据
│   └── gen_interest_data.py # 兴趣演化数据生成脚本
├── KuaiRand-1K/             # 数据集说明文件
├── README.md
└── PROJECT_DESIGN.md
```
